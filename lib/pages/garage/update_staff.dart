...
class UpdateStaffGarage extends StatefulWidget {
  String id;
  UpdateStaffGarage({super.key, required this.id});

  @override
  State<UpdateStaffGarage> createState() => _UpdateStaffGarageState();
}

class _UpdateStaffGarageState extends State<UpdateStaffGarage> {
  Timer? _timer;
  ...
  ///////////// Start Get Staff Data for Update ////////////
  Future<dynamic> getStaffData() async {
    // print(widget.id);
    var client = http.Client();
    try {
      _timer?.cancel();
      ...
      var res = await client.get(Uri.https('creativeparkingsolutions.com', 'admin/get_staff_for_update_app/${widget.id}'));
  ...
  //////////// End Get Staff Data for Update /////////////

  List _stateCityList = [];
  
  Future<void> update_staff()async{
    ...
    try {
      ...
      var res = await client.post(Uri.https('creativeparkingsolutions.com', 'admin/update_staff_app/${widget.id}'), body: {
  ...
  void initState() {
    ...
    getStaffData().then((value) {
      setState(() {
        selectedWorkType = value['work_type'];
        staff_first_name.text = value['first_name'];
        staff_last_name.text = value['last_name'];
        email.text = value['email'];
        selectedStateValue = value['state'];
        selectedCityValue = value['city'];
                    ...
                    delegate: SliverChildListDelegate(
                      [
                        Container(
                          ...
                          Column(
                            children: [
                              Text("Update Staff", style: TextStyle(fontSize: 20, color: Colors.black54, fontWeight: FontWeight.bold),),
                              SizedBox(height: 5,),
                              Form(
                                key: _formKey,
                                child: 
                                      ...
                                      child:Padding(
                                        ...
                                        DropdownWithSearch(
                                          ...
                                          onChanged: (value) async{
                                            ...
                                            String data = await DefaultAssetBundle.of(context).loadString("assets/json/state_city.json");

                                            var jsonData = jsonDecode(data);

                                            setState(() {
                                              selectedStateValue = value;
                                              _stateCityList = jsonData[value];
                                            });
