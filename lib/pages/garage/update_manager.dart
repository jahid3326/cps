...
class UpdateManagerGarage extends StatefulWidget {
  String id;
  UpdateManagerGarage({super.key, required this.id});

  @override
  State<UpdateManagerGarage> createState() => _UpdateManagerGarageState();
}

class _UpdateManagerGarageState extends State<UpdateManagerGarage> {
  Timer? _timer;
  ...
  ///////////// Start Get Client Data for Update ////////////
  Future<dynamic> getManagerData() async {
    // print(widget.id);
    var client = http.Client();
    try {
      _timer?.cancel();
      ...
      var res = await client.get(Uri.https('creativeparkingsolutions.com', 'garage/get_manager_for_update_app/${widget.id}'));
  ...
  //////////// End Get Client Data for Update /////////////
  ...
  Future<void> update_manager()async{
    ...
    try {
      ...
      var res = await client.post(Uri.https('creativeparkingsolutions.com', 'garage/update_manager_app/${widget.id}'), body: {
  ...
  void initState() {
    ...
    getManagerData().then((value) {
      setState(() {
        _first_name.text = value['first_name'];
        _last_name.text = value['last_name'];
        _user_name.text = value['user_name'];
        _user_email.text = value['email'];
        selectedGarageValue = value['G_id'];
      });
    });
                  ...
                  delegate: SliverChildListDelegate(
                    [
                      Container(
                        ...
                        Column(
                          children: [
                            Text("Update Manager", style: TextStyle(fontSize: 20, color: Colors.black54, fontWeight: FontWeight.bold),),
                            SizedBox(height: 5,),
                            Form(
                              key: _formKey,
                              child: 
