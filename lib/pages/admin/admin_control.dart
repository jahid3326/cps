...
class AdminControlAdmin extends StatefulWidget {
  const AdminControlAdmin({super.key});

  @override
  State<AdminControlAdmin> createState() => _AdminControlAdminState();
}

class _AdminControlAdminState extends State<AdminControlAdmin> {
  Timer? _timer;
  ...
  Future<dynamic> generateTableList() async {
    ...
    try {
      ...
      var res = await client.get(Uri.https('creativeparkingsolutions.com', 'admin/admin_list_app'));
...
class TableList {
  ...
  TableList({this.id, this.name, this.email, this.roll});
